...
研究上，主题熟悉度比年龄和媒体偏好更值得控制支架。但在后续产品讨论中，第一版已经明确不再要求用户逐篇回答熟悉度，以免把判断负担重新推回用户。当前落地采用分层作品：先给概览和必要前置，再给核心解释与可展开深入内容；熟悉度只作为非阻塞的可选调整。年龄只用于语言、案例、信息密度、可访问性和未成年人保护，不用于推断能力或固定学习方式。
...
每篇文章默认不再强制询问动态条件：系统直接继承个人 Skill 的默认目标，并用分层内容兼容不同熟悉度。目标、熟悉度和“8、20、35 分钟”等只作为生成后可见、可快速调整的非阻塞选项；用户不选择也能直接得到完整作品。
